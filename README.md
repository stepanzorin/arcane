![Arcane Game Engine logo](https://github.com/stepanzorin/arcane/blob/master/logo.jpg)

Arcane — Vulkan API-based game engine on **C++23**.
...
### Tools
- **[LLVM Clang](https://github.com/llvm/llvm-project/releases/tag/llvmorg-19.1.0) `>=` 19.1.0 — `Clang` is main compiler.**
- **[Python](https://www.python.org/downloads/) `>=` _3.11.5_**
- **[Visual Studio 17 2022](https://visualstudio.microsoft.com/) _(Toolset — v143)_**
- **[Vulkan SDK](https://vulkan.lunarg.com/home/welcome) `>=` 1.3.290.0**
- **[СMake](https://cmake.org/download/) `>=` _1.3.29.0-rc-2_**
- **Ninja**
```
pip install ninja
```
...
The documentation: https://pre-commit.com/
### `pre-commit`:
```
...
**Two profiles need to be created** 

## Windows

1) `win64-debug-clang-cl`
```
conan profile new win64-debug-clang-cl --detect
```
...
2) `win64-release-clang-cl`
```
conan profile new win64-release-clang-cl --detect
```
, with the following settings:
...
To start working, enter the following command:

- **Debug**
```
conan install . --profile:host win64-debug-clang-cl --profile:build win64-release-clang-cl --build=missing
```

- **Release** 
```
conan install . --profile:host win64-release-clang-cl --profile:build win64-release-clang-cl --build=missing
```

# F.A.Q.

**Post-LLVM Installation**: Ensure that the path to the folder containing clang-cl.exe is specified in the PATH environment variable after installing LLVM.

**Boost Library and clang-cl Issues**: In Windows, if there are any compatibility issues between the Boost library and clang-cl, you may need to make two modifications in the Boost recipe which doesn't consider clang-cl.

My issue: https://github.com/conan-io/conan-center-index/issues/18302

### Boost 1.85.0 for clang-win

### _.../export/conanfile.py_

<details><summary>Click to expand recipe</summary>
